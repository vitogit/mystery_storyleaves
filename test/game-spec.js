describe('Game spec', function() {
  ...
  context('Mount without params', function() {
    ...
    it('has enigma_resources_count equal to 3', function() {
      expect(tag.enigma_resources_count).to.exist;
      expect(tag.enigma_resources_count).to.be.eq(3)
    })    
  ...
  describe('Mechanics', function() {
    ...
    context('Phase 0', function() {
      ...
      // })
      //
      // it('choose a victim character ', function() {
      //
      // })
      ...
      describe('select the resources', function() {
        ...
        it('select the characters relationships', function() {
          ...
          expect(tag.resources.enigma_clue.card.text).to.exist
          expect(tag.resources.enigma_where.card.text).to.exist
          expect(tag.resources.enigma_what.card.text).to.exist
          expect(tag.areas.main.cards).to.have.lengthOf(mainDeckLength-3)
      ...
      describe('Movements', function() {
        ...
        it('run move: pursue goal (and succeed)', function() {
          ...
          var victimCard = new Card(5,'Aspecto:victim card')
          tag.loadArea('hand', [myCard])
          tag.areas.main.cards = [victimCard]
          // sinon.mock(tag.areas.main).expects("topCard").returns(victimCard) also could work with a mock
        ...
        it('run move: attack victim', function() {
          var myCard = {number:1, text:'Aspecto: my card'}
          var victimResourceCard = {number:5, text:'Aspecto:victim card'}
          tag.loadArea('hand', [myCard])
          tag.resources.pj_object.card = victimResourceCard
          tag.selectedCard = myCard
          tag.selectedVictimResource = 'pj_object'
          tag.doMove('attack')
          expect(tag.areas.hand.cards).to.have.lengthOf(0)
          expect(tag.resources.pj_object.card.text).to.be.undefined
          expect(tag.tags.chat.messages).to.have.lengthOf(1)
        ...
        it('run move: reverse condition (and lost)', function() {
          ...
          var victimCard = new Card(5,'Aspecto:victim 5 card')
          tag.loadArea('hand', [myCard])
          tag.areas.main.cards = [victimCard]
          tag.selectedCard = myCard
          tag.doMove('reverse')
